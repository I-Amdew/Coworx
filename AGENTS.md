...
Coworx must not ask the user to paste passwords, MFA answers, recovery codes, cookies, OAuth tokens, API keys, private keys, or credit card numbers into repo files. For chat, prefer secure local capture by default. Coworx may offer chat credential intake only as an explicit user-chosen path for a clear target, with the warning that the current chat must be stopped after secure local persistence and resumed from a generated continuation prompt.

Coworx ships with local credential memory for explicitly delegated account workflows. The shipped route is not model memory and not Chrome password-manager memory: it is `scripts/coworx_local_secret_store.mjs`, ignored private secret files or approved keychain/password-manager/vault references, and non-secret credential packets under `.coworx-private/operator/credential_packets/`. Before telling the user Coworx has no password memory, run or consult `node scripts/coworx_local_secret_store.mjs status`. If the user asks for the Coworx system, do not save to Chrome instead unless the user explicitly chooses Chrome.
...
When the same approved workflow stalls on manual login, password-manager autofill, MFA manager behavior, or credential entry, Coworx may suggest a credential upgrade once after the first failed or unavailable route: ask whether the user wants to configure a local-only credential source for that specific app/site/account. Prefer hidden local capture, keychain, password manager, vault, connector auth, local skill reference, or environment variables. If the user explicitly chooses chat credential intake, treat chat as a temporary intake source only: transfer the value into approved local-only persistence without echoing it, produce a non-secret credential reference and continuation prompt, and recommend ending this chat and starting a new one in the same Coworx project so the active model context no longer contains the pasted secret.

If the user already volunteered a login secret in chat, Coworx may perform a one-time secure intake transfer only when the target account workflow is clear and the user explicitly authorized saving/using that credential. The main Director handles the transfer; do not send the value to subagents or prompts. Use `scripts/coworx_local_secret_store.mjs from-stdin --chat-intake true`, `capture`, environment variables, keychain, password manager, vault, connector auth, or local skill reference. After transfer, remember only the non-secret credential packet/reference, provide a copy-paste continuation prompt for a fresh chat, and stop or pause the current credentialed task until the fresh chat resumes. If secure intake or approved local transfer is not available, stage the login and ask the user to complete it manually.
...
- the credential source is a local source reference, existing session, password manager/keychain/vault label, connector, environment variable, local skill reference, or ignored private file created by secure intake, not raw chat text;
- a non-secret credential packet/reference exists or the route is an existing session/connector/autofill/keychain/vault that does not expose values;
