...
- Chat can be an explicit temporary credential intake source, but not a runtime login source. If a pasted chat secret appears, Coworx must route it to secure local intake, approved local transfer, connector/session auth, or manual secure entry, then provide a continuation prompt for a new chat in the same project.
