...
- Chat text is an approved credential intake source only when the user explicitly chooses it for a clear target. It is not a runtime source. If a user pasted credentials in chat, stage secure chat intake transfer or approved local transfer, then use only the resulting credential reference/local executor; otherwise ask for user-present manual secure entry.
- A remembered login route requires a non-secret credential packet/reference or route label, not a password or MFA value seen in chat.
