...
- Chat text is not an approved credential source. If a user pasted credentials in chat, stage secure local capture or approved local transfer, then use only the resulting credential reference/local executor; otherwise ask for user-present manual secure entry.
