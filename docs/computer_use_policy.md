...
When the user asks to use Coworx credential memory, the Operator should use or stage `scripts/coworx_local_secret_store.mjs`, not Chrome password saving. Browser password-manager prompts remain allowed only when the user chooses that route or when it is already the approved source.
