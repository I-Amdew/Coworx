...
Coworx's shipped credential memory is the local secret-store/reference-packet flow, not Chrome's password manager and not model memory. If the user asks for Coworx to remember a password, first check `scripts/coworx_local_secret_store.mjs status`, then use the Coworx route or stage secure local capture. Do not say Coworx has no password system and then save to Chrome unless the user explicitly chooses Chrome.
